...
- Нужно зафиксировать обращение для истории""",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": """Отметить обращение как РЕШЁННОЕ AI, когда:
- Пользователь благодарит за помощь ("спасибо", "благодарю", "рахмет", "помогло")
- Пользователь подтверждает что проблема решена ("всё работает", "разобрался", "понял", "получилось")
- Пользователь говорит что вопрос закрыт ("больше вопросов нет", "это всё", "достаточно")
- AI успешно предоставил полный ответ из базы знаний и пользователь доволен

НЕ вызывай если:
- Пользователь всё ещё задаёт вопросы
- Проблема не решена
- Пользователь недоволен или жалуется
- Требуется помощь оператора""",
            "parameters": {
                ...
                "properties": {
                    "resolution_summary": {
                        "type": "string",
                        "description": "Краткое описание как была решена проблема"
                    },
                    "user_satisfaction": {
                        "type": "string",
                        "enum": ["positive", "neutral", "unknown"],
                        "description": "Оценка удовлетворённости пользователя"
                    },
                    "resolution_type": {
                        "type": "string",
                        "enum": ["faq_answer", "instruction_provided", "issue_clarified", "self_resolved"],
                        "description": "Тип решения: ответ из FAQ, предоставлена инструкция, вопрос прояснён, пользователь сам разобрался"
                    }
                },
...
4. mark_resolved_by_ai - отметить что ты успешно решил проблему
...
ОБЯЗАТЕЛЬНО используй mark_resolved_by_ai если:
- Пользователь благодарит тебя ("спасибо", "благодарю", "рахмет", "круто", "отлично")
- Пользователь подтверждает что проблема решена ("работает", "получилось", "разобрался", "понял")
- Пользователь говорит что вопросов больше нет
- Ты дал полный ответ и пользователь доволен
...
💬 Ваша заявка находится в работе. Мы уведомим вас о любых обновлениях."""
    ...
    async def _mark_resolved_by_ai(
        ...
        """Отметить обращение как решённое AI."""
        
        resolution_summary = args.get("resolution_summary", "Вопрос решён")
        user_satisfaction = args.get("user_satisfaction", "positive")
        resolution_type = args.get("resolution_type", "faq_answer")
        
        # Генерируем ID резолюции
        resolution_id = f"RES-{datetime.now().strftime('%y%m%d')}-{uuid.uuid4().hex[:4].upper()}"
        
        resolution_type_labels = {
            "faq_answer": "Ответ из FAQ",
            "instruction_provided": "Предоставлена инструкция", 
            "issue_clarified": "Вопрос прояснён",
            "self_resolved": "Пользователь разобрался самостоятельно",
        }
        resolution_type_labels_kz = {
            "faq_answer": "FAQ-дан жауап",
            "instruction_provided": "Нұсқаулық берілді",
            "issue_clarified": "Сұрақ түсіндірілді", 
            "self_resolved": "Пайдаланушы өзі шешті",
        }
        
        if language == "kz":
            message = f"""✅ **Өтініш шешілді!**

🤖 **AI арқылы шешілді**
📋 **Шешім:** {resolution_summary}
📊 **Шешім түрі:** {resolution_type_labels_kz.get(resolution_type, resolution_type)}

Көмегім үшін рахмет! Басқа сұрақтар болса, қайта хабарласыңыз. 😊"""
        else:
            message = f"""✅ **Обращение решено!**

🤖 **Решено с помощью AI**
📋 **Решение:** {resolution_summary}
📊 **Тип решения:** {resolution_type_labels.get(resolution_type, resolution_type)}

Рад был помочь! Если возникнут другие вопросы — обращайтесь. 😊"""
